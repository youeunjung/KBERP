...
<head>
<meta charset="UTF-8">
<title>카카오뱅크 ERP - 지출결의서 작성</title>
<!-- 헤더추가 -->
...
$(document).ready(function() {
	...
	$("#WriteBtn").on("click", function() {
		...
		$("#expns_type_select").val($("#expns_type option:selected").val());
						...
						success : function(res) { 
							if(res.res == "success") {
								if($("#backCheck").val() == "0") {
									...
									$("#backForm2").attr("action", "expnsRsltnEmpMnthlyList");
									$("#backForm2").submit();
								}
							} else {
...
<form action="#" id="acntSrchForm" method="post">
	...
	<form action="expnsRsltnEmpMnthlyList" id="backForm2" method="post">
		<input type="hidden" name="top" value="${param.top}">
		<input type="hidden" name="menuNum" value="${param.menuNum}">
		<input type="hidden" name="menuType" value="${param.menuType}">
		<input type="hidden" id="mon" name="mon" value="${param.mon}">
	...
	<div class="cont_wrap">
		<div class="page_title_bar">
			<div class="page_title_text">지출결의서 작성</div>
		</div>
		<!-- 해당 내용에 작업을 진행하시오. -->
		<div class="cont_area">
			<!-- 여기부터 쓰면 됨 -->
		...
		<input type="hidden" id="expns_type_select" name="expns_type_select">
			<table class="expns_rsltn_dtl_view">
				...
				<tr>
		<td>계정명</td>
		<td><input type= "text" id = "acntCodeInput" readonly="readonly"><div class= "cmn_btn_ml" id = "srchBtn">계정명 검색</div></td>
		</tr>
		<tr>
		<td>지출처</td>
		<td><input type="text" id = "expns" name ="expns"></td>
		</tr>
		<tr>
		<td>지출금액</td>
		...
		<tr>
		<td>지출일시</td>
		<td><input type="date" id ="date_hr" name ="date_hr"></td>
		</tr>
		<tr>
		<td>지출유형</td>
		<td><select id = "expns_type" name="expns_type">
		...
		<tr>
		<td>비고</td>
		<td><input type="text" id = "rmrks" name="rmrks"></td>
		</tr>
		<tr>
		<td>첨부파일</td>
		<td>
		...
		<input type="hidden" id="attFile" name="attFile">
			...
			<div class="btn_wrap">	
					...
					<div class="cmn_btn_ml" id="CencleBtn">돌아가기</div>			
			</div>
		</div>
	</div>
	<!-- bottom -->
